...
export const AiAnalysisChatbox: React.FC<AiAnalysisChatboxProps> = ({ formSpec, responses, editKey }) => {
  ...
  const [isAiRespondingToUserQuery, setIsAiRespondingToUserQuery] = useState<boolean>(false);
  ...
  const performFullAutomatedAnalysis = useCallback(async () => {
    ...
    try {
        ...
        const automatedAnalysisContextPrompt = `You are an expert in data analysis. Analyze the following form data and give summarization.`;
  ...
  }, [formSpec, responses, editKey, addMessage, chatMessages.length, loadingInitialAnalysis, error]);
